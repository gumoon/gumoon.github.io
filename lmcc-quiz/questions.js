const QUESTIONS = [
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "A 正确：GQA 中 num_q_heads/num_kv_heads 得到每组的 Q头数量；repeat_interleave沿 dim=1 逐元素重复，使每个KV头被相邻的Q头共享\nB错误：分组大小应为 q_heads/kv_heads 而非 kv_heads/q_ heads；repeat会整体重复而非逐元素交错重复，分组对应关系会错乱\nC错误：expand 不能直接将 (B,num_kv_heads,L,d) 扩展为(B,num_q_heads,L,d)， 因为 num_kv_heads ≠ num_q_heads时维度不匹配\nD错误：分组大小应为除法而非乘法，q_heads*kv_heads 会导致过度重复近似）",
    "category": "模型架构"
  },
  {
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "A正确：文本是离散符号序列，遮盖后需要预测离散符号；图像是连续像素，遮盖后重构连续像素。\nB 错误：MAE（MAE - Masked Autoencoders）的遮盖比例可以非常高（例如75%）。\nC错误：文本的掩码预测（如BERT）非常依赖全局上下文。\nD错误：MAE是重构，文本MLM也是重构。○考核方式：概念、公式、简答码（如 BERT 的 MLM）",
    "category": "预训练技术"
  },
  {
    ...
    "answer": "A",
    "explanation": "心训练任务，模型根据已有的前文序列预测下一个词元。B选项描述的是掩码语言模型（如 BERT）的训练方式。C 和D选项都不是标准的语言模型训练方式。习的关系",
    "category": "预训练技术"
  },
  {
    ...
    "answer": "B",
    "explanation": "B 正确：多任务学习是指模型在训练过程中同时学习多个相关任务，以期通过共享表示（参数）来提高整体性能和泛化能力。对模型训练的影响",
    "category": "预训练技术"
  },
  {
    ...
    "answer": "B",
    "explanation": "tch梯度估计的期望等于真实梯度，与batch size无关。较大的批量大小能提供方差更小的梯度估计（因为平均了更多样本的梯度，统计上更稳定），而较小的批量大小梯度噪声更大（方差大）。小batch的噪声有时反而有助于逃出局部最优、提升泛化能力。因此B选项中\"更加准确\"这一说法不严谨，是错误的。其他选项都正确：大批量提高并行效率（A）、需要权衡（C）、过大批量可能影响泛化（D）。",
    "category": "预训练技术"
  },
  {
    ...
    "answer": "A",
    "explanation": "始情况：256 × 4 = 1024。当批次大小增至 512 时，为保持等效批次大小 1024 不变，需要调整累积步数：1024 ÷ 512 = 2 步。梯度累积是一种在显存受限时模拟大批次训练的技术，通过累积多个小批次的梯度后再更新参数。率预热（Warm-up）",
    "category": "预训练技术"
  },
  {
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "A正确：在单次前向和反向传播过程中，至少需要存储模型参数（P）和计算过程中产生的梯度（G）。优化器的状态（O）通常在参数更新时使用，不一定在每次前向 / 反向计算时都需要全部加载到显存（但如果考虑整个 batch 的训练，则需要）。在不考虑激活值、缓存等的情况下，参数和梯度是基础。",
    "category": "预训练技术"
  },
  {
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "术。它用 float16 进行前向和反向传播计算以加速训练，同时保留 float32 的主参数副本以维持精度；并使用损失缩放（loss scaling）技术防止小梯度下溢。这种方法不能完全消除精度损失（\nB错误），不涉及整型计算（\nC错误），也不增加参数量（\nD错误）。",
    "category": "预训练技术"
  },
  {
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "A 正确：BF16 (Brain Floating Point) 格式与 FP32 一样，都拥有 8 位指数（Exponent）和 23 位尾数（Mantissa）。BF16 格式与 FP32 有相同的指数位（8 位），这意味着它能表示的数值范围与 FP32 几乎相同，这有助于避免溢出和下溢问题；但它只有 7 位尾数，相比 FP32 的 23 位尾数，其数值精度较低。FP16则有5位指数和10位尾数。计算、混合精度训练的流程、融合算子的原理",
    "category": "预训练技术"
  },
  {
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "A错误：Gradient Checkpointing 主要目的是减少显存占用，通过重计算来换取显存，它不会减少通信开销，反而可能因为引入额外的计算图而间接增加一些同步开销。B、C、D 都是通信优化的范畴：All-Reduce 是高效的聚合梯度通信算法；ZeRO 通过分片减少了需要传输的梯度和参数量；Token Compression 理论上可以减少需要传输的 token数量（如果是在模型间传输某些中间表示）。指令微调",
    "category": "预训练技术"
  },
  {
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "有用性和诚实性（3H 原则），还包括语言和文化适应性、道德规范、隐私保护等多个方面。不同的应用场景和文化背景可能对对齐标准有不同要求，因此 A、B、C 都是重要的对齐标准。",
    "category": "人类对齐"
  },
  {
    ...
    "answer": "C",
    "explanation": "C 错误：道德标准往往具有地域性、时代性和文化相关性，不存在一套全球完全统一且永恒不变的道德准则。对齐工作需要根据目标用户群体和法律环境动态调整。A、B、D 均属于对齐过程中需要考虑的多维度标准。基于排序的人类反馈",
    "category": "其他"
  },
  {
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "DPO 将奖励函数与最优策略的关系进行了闭式解变换，使得我们可以直接利用偏好数据进行监督学习（通常是交叉熵损失），从而避开了复杂的强化学习（PPO）框架。对比、DPO 模型的变种（token-level DPO 和 reference-free DPO算法）",
    "category": "人类对齐"
  },
  {
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "RAG 通过在生成前从外部可靠知识库检索相关文档并作为上下文输入给模型，显著降低了模型因知识遗忘或缺失而产生的事实性幻觉。CoT 主要解决推理逻辑问题，不能直接提供模型未学过的实时事实。解码与部署",
    "category": "人类对齐"
  },
  {
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "A正确：温度采样的公式为 logits/T，T越大分布越平滑（更随机），T 越小越尖锐（更确定）；Top-P 先用 cumulative_ probs > top_p 标记超过阈值的位置，再将掩码右移一位，从而保留第一个超过阈值的 token，得到“累计概率刚好覆盖top_p”的最小候选集合\nB错误：温度应该是除法（logits/T）而非乘法，乘以温度会使高温时分布更尖锐，与期望相反\nC错误：Top-P 应移除累积概率超过 p 的 token（>p），而非低于p的（<p）\nD错误：温度缩放是除法操作，不是减法；且应基于累积概率而非单个token概率来过滤",
    "category": "解码与部署"
  },
  {
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "型的知识，通过设计合适的提示来引导模型完成特定任务，无需大量标注数据和重新训练。因此最适合B选项的场景。A 选项需要从零训练，C 和 D 选项不是提示学习的典型应用场景。的使用",
    "category": "其他"
  },
  {
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "Profile（画像 / 身份）设置通过提示词工程，让模型在特定的语境（Context）下运行，使其回复更符合特定角色（如：资深医生、幽默导游）的预期行为。",
    "category": "提示学习"
  },
  {
    ...
    "answer": "A",
    "explanation": "由于模型注意力机制的限制，长对话中初始设定权重会下降。通过“固定前缀”或在每轮输入中动态注入 Profile 信息（Prompt Injection），可以有效维持角色的稳定性。记忆的存储和读取",
    "category": "智能体"
  },
  {
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "A 正确：MMLU 是英文多学科评测基准，C-Eval 是中文多学科评测基准\nB错误：不同评测集侧重不同能力维度\nC错误：BIG-Bench包含200+多样化任务，涵盖推理、翻译、常识等\nD错误：HELM是全面的评测框架，关注准确性、鲁棒性、公平性等多个维度",
    "category": "人工智能基础概念"
  },
  {
