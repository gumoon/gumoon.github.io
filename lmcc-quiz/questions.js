const QUESTIONS = [
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "A 正确：KV Cache 缓存历史的 Key 和 Value，每步只需计算新token的注意力，大幅减少计算量B 错误：全量解码每步重复计算所有位置，效率远低于增量解码C 错误：vLLM 是高效的 LLM 推理引擎， 通过PagedAttention等技术加速推理D错误：解码效率评估包括吞吐量（tokens/s）、延迟（latency）、\n首token时间（TTFT）等指标回归解码、早退机制、级联解码）、解码加速的系统级优化\n（FlashAttention、PagedAttention、批次管理优化）",
    "category": "大模型基础概念"
  },
  {
