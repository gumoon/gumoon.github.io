const QUESTIONS = [
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "A正确：下一个词元预测是自回归语言模型（如 GPT）的核心训练任务，模型根据已有的前文序列预测下一个词元。B选项描述的是掩码语言模型（如 BERT）的训练方式。C和D选项都不是标准的语言模型训练方式。",
    "category": "预训练技术"
  },
  {
  ...
  },
  {
    ...
    "answer": "A",
    "explanation": "A错误：Gradient Checkpointing 主要目的是减少显存占用，通过重计算来换取显存，它不会减少通信开销，反而可能因为引入额外的计算图而间接增加一些同步开销。B、C、D 都是通信优化的范畴：All-Reduce 是高效的聚合梯度通信算法；ZeRO 通过分片减少了需要传输的梯度和参数量；Token Compression 理论上可以减少需要传输的 token数量（如果是在模型间传输某些中间表示）。",
    "category": "预训练技术"
  },
  {
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "C 错误：道德标准往往具有地域性、时代性和文化相关性，不存在一套全球完全统一且永恒不变的道德准则。对齐工作需要根据目标用户群体和法律环境动态调整。A、B、D 均属于对齐过程中需要考虑的多维度标准。基于排序的人类",
    "category": "其他"
  },
  {
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "B正确：RAG 通过在生成前从外部可靠知识库检索相关文档并作为上下文输入给模型，显著降低了模型因知识遗忘或缺失而产生的事实性幻觉。CoT 主要解决推理逻辑问题，不能直接提供模型未学过的实时事实。",
    "category": "人类对齐"
  },
  {
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "B正确：提示学习（Prompt Learning）的核心思想是利用预训练模型已学到的知识，通过设计合适的提示来引导模型完成特定任务，无需大量标注数据和重新训练。因此最适合B选项的场景。A选项需要从零训练，C和D选项不是提示学习的典型应用场景。",
    "category": "其他"
  },
  {
