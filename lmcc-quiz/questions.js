const QUESTIONS = [
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "B错误：Batch梯度估计的期望等于真实梯度，与batch size无关的期望等于真实梯度，与batch size无关。较大的批量大小能提供方差更小的梯度估计（因为平均了更多样本的梯度，统计上更稳定），而较小的批量大小梯度噪声更大（方差大）。小batch的噪声有时反而有助于逃出局部最优、提升泛化能力。因此B选项中\"更加准确\"这一说法不严谨，是错误的。其他选项都正确：大批量提高并行效率（A）、需要权衡（C）、过大批量可能影响泛化（D）。",
    "category": "预训练技术"
  },
  {
    ...
    "answer": "A",
    "explanation": "A正确：初始情况：256 × 4 = 1024。当批次大小增至 512 时，为保持等效批次大小 1024 不变，需要调整累积步数：1024 ÷ 512 = 2 步。梯度累积是一种在显存受限时模拟大批次训练的技术，通过累积多个小批次的梯度后再更新参数。",
    "category": "预训练技术"
  },
  {
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "B正确：梯度裁剪通过限制梯度的范数或值，防止训练中出现梯度爆炸问题，这在训练循环神经网络和深层网络时尤为重要。它不直接加速收敛（\nA错误）、不减少参数量（\nC错误）、主要目的也不是防止过拟合（\nD错误），而是确保训练过程的数值稳定性。",
    "category": "其他"
  },
  {
    ...
    "answer": "A",
    "explanation": "A正确：混合精度训练是一种加速训练的技术。它用 float16 进行前向和反向传播计算以加速训练，同时保留 float32 的主参数副本以维持精度；并使用损失缩放（loss scaling）技术防止小梯度下溢。这种方法不能完全消除精度损失（\nB错误），不涉及整型计算（\nC错误），也不增加参数量（\nD错误）。",
    "category": "预训练技术"
  },
  {
  ...
  },
  {
    ...
    "answer": "D",
    "explanation": "D正确：人类对齐除了核心的无害性、有用性和诚实性（3H 原则）（3H 原则），还包括语言和文化适应性、道德规范、隐私保护等多个方面。不同的应用场景和文化背景可能对对齐标准有不同要求，因此 A、B、C 都是重要的对齐标准。",
    "category": "人类对齐"
  },
  {
  ...
  },
  {
    ...
    "answer": "C",
    "explanation": "    "explanation": "C正确：提示工程是在不改变模型参数的前提下引导模型输出。其局限性在于对输入的敏感性（Robustness 问题），微小的改动可能导致截然不同的结果。",",
    "category": "提示学习"
  },
  {
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "B正确：上下文学习（In-Context Learning）是通过输入中的示例（Input-Output Pairs）进行类比推理。",
    "category": "大模型基础概念"
  },
  {
    ...
    "answer": "B",
    "explanation": "B正确：思维链（CoT）通过生成中间推理步骤，降低了模型直接从问题跳转到复杂答案的推理难度。跳转到复杂答案的推理难度。",
    "category": "提示学习"
  },
  {
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "B正确：Profile（画像 / 身份）设置通过提示词工程，让模型在特定的语境（Context）下运行，使其回复更符合特定角色（如：资深医生、幽默导游）的预期行为。",
    "category": "提示学习"
  },
  {
  ...
  },
  {
    ...
    "answer": "B",
    "explanation": "B正确：有效的记忆机制需要平衡“完整性”和“效率”。摘要化（将长对话浓缩成关键信息）和向量化检索（基于语义召回相关记忆）是解决长时记忆的主流技术。",
    "category": "其他"
  },
  {
    ...
    "answer": "B",
    "explanation": "B正确：LLM 在工具使用中充当“大脑”和“调度器”，它负责识别何时需要工具，并按预定义格式输出参数。实际的执行由外部系统完成。",
    "category": "智能体"
  },
  {
